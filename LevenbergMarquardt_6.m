function [i, xk] = LevenbergMarquardt_6(f, grad, x0, iterations, tol, m_pk)
...
for i = 1:iterations
    ...
    if e_lm<e
        ...
        e = e_lm;
        xk = xk1;
        if  abs(grad(xk1)) < (0.25 / muk)  % gnorm→JF
            muk = muk * 1;
            num = 1;
        elseif abs(grad(xk1)) < (0.75 / muk)
            muk = muk * 0.4;
            num = 2;
        else
            muk = max(muk * 0.25, 1e-04);
        end
        lamda = muk * d;
%         xk = xk1;
...
%     fprintf("iter=%d, updateJ=%d, gnorm=%f, rk=%f, muk=%f, d=%f, lamda=%f, num=%d\n",i, updateJ, gnorm, rk, muk, d, lamda, num);
