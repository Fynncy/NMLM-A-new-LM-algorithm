function [xlog, ylog, xdlog, i, xk] = LevenbergMarquardt_m1(f, grad, x0, iterations, tol, hessian)
...
%         disp(e);
        ...
        func_xk =  grad(xk);
        func_xk1 =  grad(xk1);
        rho = sum(Powell(xk1)-Powell(xk)) / sum(g*pk + lamda*pk'*H*pk); %GG(X) @ d + 0.5 * d.T @ HH(X) @ d         
%         rho = (func_xk'* func_xk - func_xk1'* func_xk1) / (pk' * (lamda * pk - g));
%         rk = norm(rho);
%         xk = xk + alpha * pk;
        fprintf("iter=%d, updateJ=%d, rho=%f, lamda=%f\n", i, updateJ, rho,lamda);
        rk =rho;
        disp(rk);
        if rk < 0.25
            lamda = lamda * 10;
